...
public class EmployeeImpl implements EmployeeRepository {
    ...
    public int createEmployee(Employee employee) {
        var sql ="INSERT INTO employee(name,email,mobile,location,department_id) VALUES(?,?,?,?,?)";
        return jdbcTemplate.update(sql,new Object[]{employee.getName(),employee.getEmail(),employee.getMobile(),employee.getLocation(),employee.getDepartment_id()
        });
    }

    @Override
    public int updateEmployee(Employee employee,int id) {
        var sql="UPDATE employee set name=?,email=?,mobile=?,location=?,department_id=? WHERE id=?";
        return jdbcTemplate.update(sql,employee.getName(),employee.getEmail(),employee.getMobile(),employee.getLocation(),employee.getDepartment_id(),id);
    }
